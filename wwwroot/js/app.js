...
const generatePattern = (hue) => {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const color = `hsla(${h}, 60%, 50%, ${Math.random() * 0.15 + 0.05})`;
        ...
        if (type < 0.4) {
            svgContent += `<circle cx="${x}" cy="${y}" r="${size}" fill="${color}" />`;
        } else if (type < 0.8) {
            svgContent += `<rect x="${x - size / 2}" y="${y - size / 2}" width="${size}" height="${size}" fill="${color}" />`;
        } else {
            svgContent += `<polygon points="${x},${y - size} ${x + size},${y + size} ${x - size},${y + size}" fill="${color}" />`;
        }
    }

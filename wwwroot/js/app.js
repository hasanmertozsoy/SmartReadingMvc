...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            fullscreen: "Tam Ekran", exitFullscreen: "Tam Ekran Çıkış", lightMode: "Açık Mod", darkMode: "Koyu Mod",
            selectColor: "Renk Seç", delete: "Sil", minimize: "Küçült", maximize: "Büyüt", themeColor: "Tema Rengi"
        },
        en: {
            ...
            fullscreen: "Fullscreen", exitFullscreen: "Exit Fullscreen", lightMode: "Light Mode", darkMode: "Dark Mode",
            selectColor: "Select Color", delete: "Delete", minimize: "Minimize", maximize: "Maximize", themeColor: "Theme Color"
        }
    };
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button
                        ...
                        className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20"
                        aria-label={translate(settings.language, 'fav')}
                    >
                                    ...
                                    <div className="grid grid-cols-6 gap-2">
                                        {Object.keys(THEME_HUES).map(color => (
                                            <button 
                                                ...
                                                aria-label={`${translate(settings.language, 'themeColor')}: ${color}`}
                                            />
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex bg-black/10 rounded-lg p-1">
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'light' ? 'bg-white shadow text-black' : 'opacity-50'}`}
                                            aria-label={translate(settings.language, 'lightMode')}
                                        >
                                            <Sun size={16} />
                                        </button>
                                        <button
                                            onClick={() => updateSettings({ ...settings, themeMode: 'dark' })}
                                            className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'dark' ? 'bg-black shadow text-white' : 'opacity-50'}`}
                                            aria-label={translate(settings.language, 'darkMode')}
                                        >
                                            <Moon size={16} />
                                        </button>
                                    </div>

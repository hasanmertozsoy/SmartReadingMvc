...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            maximize: "Tam Ekran", minimize: "Küçült", lightMode: "Açık Mod", darkMode: "Koyu Mod",
            addNote: "Not Ekle", toggleFav: "Favorile/Kaldır", mute: "Sessize Al", unmute: "Sesi Aç",
            close: "Kapat", delete: "Sil"
        },
        en: {
            ...
            maximize: "Maximize", minimize: "Minimize", lightMode: "Light Mode", darkMode: "Dark Mode",
            addNote: "Add Note", toggleFav: "Toggle Favorite", mute: "Mute", unmute: "Unmute",
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    <button aria-label={translate(settings.language, 'close')} onClick={() => { handleSentenceFinish(currentIndexRef.current); onExit(); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20"><X size={24} /></button>
                    <button aria-label={translate(settings.language, 'toggleFav')} onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">
                    ...
                    <button aria-label={translate(settings.language, isMuted ? 'unmute' : 'mute')} onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">
                ...
                <>
                    ...
                    <header className="px-5 py-4 pb-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] flex-none flex justify-between items-center z-50 border-b border-gray-500/10 backdrop-blur-md bg-opacity-80" style={{ backgroundColor: 'var(--bg)' }}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button aria-label={translate(settings.language, isFullscreen ? 'minimize' : 'maximize')} onClick={() => { if (!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen(); }} className="p-2 opacity-70 hover:opacity-100">
                    ...
                    <div className="flex-none p-5 pb-0 bg-[var(--bg)] z-40">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <button type="button" onClick={() => startSession('fav')} aria-label={translate(settings.language, 'fav')} className="w-full text-left relative p-4 rounded-2xl cursor-pointer group hover:scale-[1.02] transition-transform shadow-sm border border-gray-500/10" style={{ backgroundColor: 'var(--card-bg)' }}>
                            ...
                            <button type="button" onClick={() => startSession('rev')} aria-label={translate(settings.language, 'rev')} className="w-full text-left relative p-4 rounded-2xl cursor-pointer group hover:scale-[1.02] transition-transform shadow-sm border border-gray-500/10" style={{ backgroundColor: 'var(--card-bg)' }}>

...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            close: "Kapat", mute: "Sessize Al", unmute: "Sesi Aç", fullscreen: "Tam Ekran",
            exitFullscreen: "Tam Ekrandan Çık", delete: "Sil", addNote: "Yeni Not Ekle"
        },
        en: {
            ...
            close: "Close", mute: "Mute", unmute: "Unmute", fullscreen: "Fullscreen",
            exitFullscreen: "Exit Fullscreen", delete: "Delete", addNote: "Add New Note"
        }
    };
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20" aria-label={translate(settings.language, 'fav')}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20" aria-label={isMuted ? translate(settings.language, 'unmute') : translate(settings.language, 'mute')}>
                ...
                <>
                    ...
                    <header className="px-5 py-4 pb-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] flex-none flex justify-between items-center z-50 border-b border-gray-500/10 backdrop-blur-md bg-opacity-80" style={{ backgroundColor: 'var(--bg)' }}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => { if (!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen(); }} className="p-2 opacity-70 hover:opacity-100" aria-label={isFullscreen ? translate(settings.language, 'exitFullscreen') : translate(settings.language, 'fullscreen')}>

 ...
 * Optimization: Replaced Canvas API with SVG string generation for performance.
 * This avoids expensive main-thread rasterization and JPEG encoding (approx. 200x faster).
...
const generatePattern = (hue) => {
    const width = 800;
    const height = 1600;
    let shapes = `<rect width="${width}" height="${height}" fill="hsl(${hue}, 30%, 10%)" />`;
    ...
    return 'data:image/svg+xml;base64,' + btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">${shapes}</svg>`);
};

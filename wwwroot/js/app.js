...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            mute: "Sesi Kapat", unmute: "Sesi Aç", close: "Kapat", favor: "Favorile", unfavor: "Favorilerden Çıkar"
        },
        en: {
            ...
            mute: "Mute", unmute: "Unmute", close: "Close", favor: "Favorite", unfavor: "Unfavorite"
        }
    };
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button aria-label={translate(settings.language, activeNoteRef.current.sentences.find(s => s.id === queue[currentIndexRef.current]?.id)?.isFavorite ? 'unfavor' : 'favor')} onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">

...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            close: "Kapat", mute: "Sessize Al", unmute: "Sesi Aç", delete: "Sil",
            fullScreen: "Tam Ekran", exitFullScreen: "Tam Ekran Çık", add: "Not Ekle",
            addToFav: "Favorilere Ekle", removeFromFav: "Favorilerden Çıkar", timer: "Zamanlayıcı"
        },
        en: {
            ...
            close: "Close", mute: "Mute", unmute: "Unmute", delete: "Delete",
            fullScreen: "Fullscreen", exitFullScreen: "Exit Fullscreen", add: "Add Note",
            addToFav: "Add to Favorites", removeFromFav: "Remove from Favorites", timer: "Timer"
        }
    };
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button aria-label={translate(settings.language, activeNoteRef.current.sentences.find(s => s.id === queue[currentIndexRef.current]?.id)?.isFavorite ? 'removeFromFav' : 'addToFav')} onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">
        ...
        <div className="h-[100dvh] w-full flex flex-col font-sans transition-colors duration-300 relative overflow-hidden">
            ...
            {currentView === 'reader' && activeNote ? (
                ...
                <>
                    ...
                    {/* Ekle Butonu */}
                    <button 
                        aria-label={translate(settings.language, 'add')}

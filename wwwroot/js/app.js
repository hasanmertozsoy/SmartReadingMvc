...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            fullscreen: "Tam Ekran", exitFullscreen: "Tam Ekrandan Çık", mute: "Sesi Kapat",
            unmute: "Sesi Aç", favAdd: "Favorilere Ekle", favRem: "Favorilerden Çıkar",
            light: "Aydınlık Mod", dark: "Karanlık Mod"
        },
        en: {
            ...
            fullscreen: "Fullscreen", exitFullscreen: "Exit Fullscreen", mute: "Mute",
            unmute: "Unmute", favAdd: "Add to Favorites", favRem: "Remove from Favorites",
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button aria-label={translate(settings.language, activeNoteRef.current.sentences.find(s => s.id === queue[currentIndexRef.current]?.id)?.isFavorite ? 'favRem' : 'favAdd')} onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">
                            ...
                            <div className="w-full max-w-sm rounded-3xl p-6 shadow-2xl" style={{ backgroundColor: 'var(--card-bg)' }}>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex bg-black/10 rounded-lg p-1">
                                        <button aria-label={translate(settings.language, 'light')} onClick={() => updateSettings({ ...settings, themeMode: 'light' })} className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'light' ? 'bg-white shadow text-black' : 'opacity-50'}`}><Sun size={16} /></button>
                                        <button aria-label={translate(settings.language, 'dark')} onClick={() => updateSettings({ ...settings, themeMode: 'dark' })} className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'dark' ? 'bg-black shadow text-white' : 'opacity-50'}`}><Moon size={16} /></button>
                                    </div>
                                </div>

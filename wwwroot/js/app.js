...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            fileError: "Dosya hatası", sentences: "Cümle", maximize: "Tam Ekran", minimize: "Küçült",
            mute: "Sessiz", unmute: "Sesi Aç", close: "Kapat", addNote: "Not Ekle", toggleFav: "Favorile",
            lightMode: "Aydınlık Mod", darkMode: "Karanlık Mod", delete: "Sil"
        },
        en: {
            ...
            fileError: "File error", sentences: "Sentences", maximize: "Maximize", minimize: "Minimize",
            mute: "Mute", unmute: "Unmute", close: "Close", addNote: "Add Note", toggleFav: "Favorite",
            lightMode: "Light Mode", darkMode: "Dark Mode", delete: "Delete"
        }
    };
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    <button onClick={() => { handleSentenceFinish(currentIndexRef.current); onExit(); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20" aria-label={translate(settings.language, 'close')}><X size={24} /></button>
                    <button onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20" aria-label={translate(settings.language, 'toggleFav')}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20" aria-label={translate(settings.language, isMuted ? 'unmute' : 'mute')}>
                ...
                <>
                    ...
                    <header className="px-5 py-4 pb-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] flex-none flex justify-between items-center z-50 border-b border-gray-500/10 backdrop-blur-md bg-opacity-80" style={{ backgroundColor: 'var(--bg)' }}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => { if (!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen(); }} className="p-2 opacity-70 hover:opacity-100" aria-label={translate(settings.language, isFullscreen ? 'minimize' : 'maximize')}>
                                {isFullscreen ? <Minimize size={20} /> : <Maximize size={20} />}
                            </button>
                            <button onClick={() => setIsSettingsOpen(true)} className="p-2 opacity-70 hover:opacity-100" aria-label={translate(settings.language, 'settings')}><Settings size={20} /></button>
                        </div>
                    </header>
                    ...
                    <div className="flex-none p-5 pb-0 bg-[var(--bg)] z-40">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <button onClick={() => startSession('fav')} className="w-full text-left relative p-4 rounded-2xl cursor-pointer group hover:scale-[1.02] transition-transform shadow-sm border border-gray-500/10" style={{ backgroundColor: 'var(--card-bg)' }} aria-label={translate(settings.language, 'fav')}>
                                <span className="flex justify-between items-start mb-2 w-full"><Heart size={24} className="text-red-500 fill-red-500" /><span className="text-2xl font-bold opacity-80">{getFavCount()}</span></span>
                                <span className="font-medium opacity-70 block">{translate(settings.language, 'fav')}</span>
                            </button>
                            <button onClick={() => startSession('rev')} className="w-full text-left relative p-4 rounded-2xl cursor-pointer group hover:scale-[1.02] transition-transform shadow-sm border border-gray-500/10" style={{ backgroundColor: 'var(--card-bg)' }} aria-label={translate(settings.language, 'rev')}>
                                <span className="flex justify-between items-start mb-2 w-full"><RefreshCcw size={24} className="text-[var(--accent)]" /><span className="text-2xl font-bold opacity-80">{getReviewCount()}</span></span>
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={e => deleteNote(e, note.id)} className="p-2 text-gray-400 hover:text-red-400 rounded-full z-10 bg-black/5" aria-label={translate(settings.language, 'delete')}><Trash2 size={16} /></button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="w-full max-w-sm rounded-3xl p-6 shadow-2xl" style={{ backgroundColor: 'var(--card-bg)' }}>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex bg-black/10 rounded-lg p-1">
                                        <button onClick={() => updateSettings({ ...settings, themeMode: 'light' })} className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'light' ? 'bg-white shadow text-black' : 'opacity-50'}`} aria-label={translate(settings.language, 'lightMode')}><Sun size={16} /></button>
                                        <button onClick={() => updateSettings({ ...settings, themeMode: 'dark' })} className={`flex-1 py-2 rounded-md flex justify-center ${settings.themeMode === 'dark' ? 'bg-black shadow text-white' : 'opacity-50'}`} aria-label={translate(settings.language, 'darkMode')}><Moon size={16} /></button>
                                    </div>
                                </div>

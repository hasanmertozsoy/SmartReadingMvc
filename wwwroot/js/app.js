...
const translate = (lang, key) => {
    const dictionary = {
        tr: {
            ...
            fileError: "Dosya hatası", sentences: "Cümle", close: "Kapat", favorite: "Favori",
            mute: "Sessiz", unmute: "Sesi Aç", lightMode: "Aydınlık Mod", darkMode: "Karanlık Mod",
            enterFullscreen: "Tam Ekran", exitFullscreen: "Tam Ekrandan Çık", color: "Renk"
        },
        en: {
            ...
            fileError: "File error", sentences: "Sentences", close: "Close", favorite: "Favorite",
            mute: "Mute", unmute: "Unmute", lightMode: "Light Mode", darkMode: "Dark Mode",
            enterFullscreen: "Enter Fullscreen", exitFullscreen: "Exit Fullscreen", color: "Color"
        }
    };
        ...
        >
            ...
            <div className={`absolute top-0 right-0 p-4 pt-[calc(env(safe-area-inset-top)+3.5rem)] transition-opacity duration-300 ${showControls && !isPaused && !isBreakMode ? 'opacity-100' : 'opacity-0'} pointer-events-none`}>
                <div className="flex flex-col items-end gap-2">
                    ...
                    <button aria-label={translate(settings.language, 'favorite')} onClick={e => { e.stopPropagation(); handleDoubleTap(queue[currentIndexRef.current]?.id); }} className="p-3 bg-black/50 rounded-full text-white pointer-events-auto hover:bg-white/20">
                ...
                <>
                    ...
                    <header className="px-5 py-4 pb-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] flex-none flex justify-between items-center z-50 border-b border-gray-500/10 backdrop-blur-md bg-opacity-80" style={{ backgroundColor: 'var(--bg)' }}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button aria-label={translate(settings.language, isFullscreen ? 'exitFullscreen' : 'enterFullscreen')} onClick={() => { if (!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen(); }} className="p-2 opacity-70 hover:opacity-100">
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button aria-label={translate(settings.language, 'deleteConfirm')} onClick={e => deleteNote(e, note.id)} className="p-2 text-gray-400 hover:text-red-400 rounded-full z-10 bg-black/5"><Trash2 size={16} /></button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="grid grid-cols-6 gap-2">
                                        {Object.keys(THEME_HUES).map(color => (
                                            <button 
                                                key={color} 
                                                aria-label={`${translate(settings.language, 'color')}: ${color}`}
